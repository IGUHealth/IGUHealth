...
This is done via the [MessageBroker](../Data_model/R4/MessageBroker) resource. This resource defines the type of broker you are using, and the connection details.

### Configuring a Message Topic
Once you have configured your message broker, you can create a [MessageTopic](../Data_model/R4/MessageTopic) resource. 
This resource defines the topic you are using to send and receive messages.

### Sending a Message
To send a message to a topic, you invoke the following Operation on the [MessageTopic](../Data_model/R4/MessageTopic) resource:
$message-post
#### Example Request
```
...
2. **Tie an AccessPolicy to your ClientApplication that allows it to post messages to the broker:**
    ```bash
    {
    "code": "admin-access",
    "name": "Admin Access",
