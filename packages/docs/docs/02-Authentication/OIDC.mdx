...
## Endpoints
To view oidc endpoints navigate to `https://<api-domain>/w/<your-tenant-id>/oidc/.well-known/openid-configuration`
* Our cloud is hosted under `https://api.iguhealth.com`

## Client Registration

To register a client, you need to make a POST request to the client registration endpoint at `https://<api-domain>/w/<your-tenant-id>/api/v1/fhir/r4/ClientApplication`.
We use a custom resource type `ClientApplication` to register clients using the FHIR API see [here](https://www.npmjs.com/package/@iguhealth/iguhealth.fhir.r4.core?activeTab=code) for more details.

## Testing

To test the OIDC endpoints you can do the following:

1. Go to `https://<your-tenant-id>.admin.iguhealth.app/` or equivalent if hosting our admin-app on your own.
