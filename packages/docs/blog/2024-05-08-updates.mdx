...
We now support external message brokers. You can now configure your own [message broker](/Data_model/R4/MessageBroker) and [message topics](/Data_model/R4/MessageTopic) to send and receive messages between your services.

### Support for pushing messages to a message broker
#### Subscription
We support tying FHIR Subscriptions to message brokers. Read our documentation [here](/Messaging/Overview) on how to set this up.
#### API
We have added an API to push messages to a message broker. You can now push messages to a message broker by invoking the following operation on the [MessageTopic](/Data_model/R4/MessageTopic) resource:
```bash
...
We have added documentation for the R4B FHIR version. You can now view the documentation [here](Data_Model/R4B/Account).
