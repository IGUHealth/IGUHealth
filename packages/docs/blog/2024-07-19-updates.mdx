...
## Terminology
We are now supporting the following external terminologies:
...
### Workers migration to HTTP
Workers are seperate processes we use for processing subscriptions and asynchronous tasks. 
...
### Conformance Testing
Finished initial round of conformance testing via [Touchstone](https://touchstone.aegis.net/touchstone). We currently have a 91% pass on the FHIR Standard R4 suite.

### Bugs
* We now allow undefined values on resources where the SD has min set to 0 for the property.
