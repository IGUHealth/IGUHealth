...
test("History test", async () => {
  ...
  try {
    const p = await client.create({}, "4.0", {
      resourceType: "Patient",
    } as Patient);

    resources.push(p);
    expect(p.id).toBeDefined();
    await client.update({}, "4.0", p.resourceType, p.id as id, p);
    ...
    const p2 = await client.create({}, "4.0", {
      resourceType: "Patient",
    } as Patient);
    resources.push(p2);
    expect(p2.id).toBeDefined();
    await client.update({}, "4.0", p2.resourceType, p2.id as id, p2);
    const typeHistory = await client.historyType({}, "4.0", "Patient", [
      { name: "_since", value: [time] },
    ]);
    expect(typeHistory.length).toEqual(4);

    const practitioner = await client.create({}, "4.0", {
    ...
    const systemHistory = await client.historySystem({}, "4.0", [
      { name: "_since", value: [time] },
    ]);
