 ...
 * OIDC Router
 */
export async function createOIDCRouter<State extends KoaState.IGUHealth>(
  prefix: string,
  {
    authMiddlewares,
    middleware,
    ...
    middleware: Router.Middleware<State, Koa.DefaultContext>[];
  },
) {
  const managementRouter = new Router<State, Koa.DefaultContext>({
    prefix,
  });
