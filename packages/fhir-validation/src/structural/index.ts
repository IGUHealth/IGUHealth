 ...
 * @returns
 */
async function validateElement2(
  ctx: ValidationCTX,
  meta: ElementNode | (FPPrimitiveNode & { definition: ElementDefinition }),
  root: object,
  path: Loc<any, any, any>,
  ...
  switch (true) {
      ...
      return (
        await Promise.all(
          (value || []).map((_v: any, i: number) => {
            return validateElement2Singular(ctx, meta, root, descend(path, i));
          }),
        )
      ).flat();
    }
    // Means single item.
    default: {
      return validateElement2Singular(ctx, meta, root, path);
    }
  }
}
 ...
 * @returns
 */
async function validateElement2Singular(
  ctx: ValidationCTX,
  meta: ElementNode | (FPPrimitiveNode & { definition: ElementDefinition }),
  root: unknown,
  path: Loc<any, any, any>,
